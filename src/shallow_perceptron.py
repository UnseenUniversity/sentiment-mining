    ...
    for e, line in enumerate(open(loc_dataset,"rb")):
        if e > 0:
            ...
            if len(r) == 3: #train set
                features = [(hash(lemmatize_word(f))%opts["D"],1) for f in r[2].split()]
                label = int(r[1])
            else: #test set
                features = [(hash(lemmatize_word(f))%opts["D"],1) for f in r[1].split()]
                label = 1
...
if __name__ == "__main__":
    ...
    with open("../data/perceptron-res.csv","wb") as outfile:
